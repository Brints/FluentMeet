...
def override_oauth_service():
    app.dependency_overrides[get_google_oauth_service] = lambda: GoogleOAuthService("id", "secret", "uri")
    yield
...
def test_google_callback_invalid_state(
    ...
    response = client.get("/api/v1/auth/google/callback?code=mockcode&state=invalidstate")
...
def test_google_callback_success(
    ...
    mock_auth_svc.resolve_oauth_user.return_value = (
        LoginResponse(access_token="test_access_jwt", user_id=uuid.uuid4(), token_type="bearer", expires_in=3600),
        "test_refresh_jwt",
        86400
    )
